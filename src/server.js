  ...
  router.post("/webhooks/inbound", async (req, res) => {
    try {
      ...
      if (result.tag === "price_request") {
        await sendWhatsAppMessage({ config, to: req.body.phone, text: config.MESSAGES.price_list });
      }

      if (result.tag === "location_request") {
        await sendWhatsAppMessage({ config, to: req.body.phone, text: config.MESSAGES.location });
      }

      if (result.tag === "optout") {
        await sendWhatsAppMessage({ config, to: req.body.phone, text: config.MESSAGES.opt_out_confirm });
      }
...
app.use((req, res) => {
  return res.status(404).json({
    ...
    hint: "Use website routes or API routes under /api/* and /api/index/*."
  });
});
